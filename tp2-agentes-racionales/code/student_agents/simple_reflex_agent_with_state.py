...
class SimpleReflexAgentWithState(BaseAgent):
    directions = {"up": (0, -1), "left": (-1, 0), "down": (0, 1), "right": (1, 0)}

    def __init__(self, server_url="http://localhost:5000", **kwargs):
        super().__init__(server_url, "SimpleReflexAgentWithState", **kwargs)
    ...
    def get_strategy_description(self):
        return """The agent should maintain some sort of internal state
                that depends on the percept history and thereby reflects
                at least some of the unobserved aspects of the current state"""
