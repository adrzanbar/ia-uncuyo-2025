...
def best_first_graph_search(problem, f, display=False):
    ...
    while frontier:
        ...
        for child in node.expand(problem):
            ...
            # Constant optimization. Reduced 4 O(n) to 2 O(n) operations.
            # For optimal performance, a data structure with at most O(log n) for lookup and deletion is needed.
            try:
                f_frontier_child = frontier[child]
            except KeyError:
                f_frontier_child = None

            if child.state not in explored and not f_frontier_child:
                frontier.append(child)
            elif f_frontier_child and f(child) < f_frontier_child:
                del frontier[child]
                frontier.append(child)
